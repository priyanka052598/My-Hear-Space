  ...
  <div className='up text-[#ffffff] flex px-[px] justify-center items-center py-20'>
      <div className="left flex flex-col justify-center items-center ">
        <div className="h text-[27px] flex flex-col justify-center items-center self-center text-center font-semibold">
          <div className='text-[#808080] text-center'>"Make a Difference by</div> <div>Listening"</div>
        </div>
       
        <div className='text-[18px] text-center text-[#d9d9d9] py-10 '>Become a listener and help people share their feelings while creating a positive impact.</div>
...
Become a Listener        </div>
 ...
 <div className={`down text-white ${isLgScreen  ? "text-[40px] pt-20" : "text-[22px] pt-10" } font-semibold`}>
  ...
  <div className={`boxes ${isLgScreen ? "flex" : "flex flex-col items-center"} flex-wrap mb-20 justify-center  gap-[24px]`}>
    {[
      { id: '01', title: 'Complete Verification', desc: 'Fill out the form and submit your application for approval' },
      { id: '02', title: 'Profile Setup', desc: 'Set up your profile with a picture and a short bio' },
      { id: '03', title: 'Training Session', desc: 'Attend a training session to learn the basics' },
      { id: '04', title: 'Start Listening', desc: 'Begin accepting requests and providing support' }
    ].map((step, index) => (
      <div key={index} className={`box text-white rounded-[16px]  ${isLgScreen ? "w-[35%] p-[40px]" : "w-[95%] px-[10px] py-[20px]" } bg-[#1f1f1f]`}>
        <div className={`index rounded-[6px] text-[24px] px-2 py-1 w-fit ${isLgScreen ? "border-[1px]" : " text-[#464646]" } border-white`}>
          <span>{step.id}</span>
        </div>
        <div className={`head ${isLgScreen ? "text-[32px]" : "text-[24px]" } py-[24px] font-semibold`}>{step.title}</div>
        <div className={`desc ${isLgScreen ? "text-[20px]" : "text-[16px]" }`}>{step.desc}</div>
      </div>
    ))}
  </div>
</div>
